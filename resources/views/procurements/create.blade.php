        ...
        <div>
            ...
            <div id="items-container" class="space-y-3">
                <div class="item-row grid grid-cols-12 gap-2 items-end bg-slate-50 p-3 rounded-xl border border-slate-200">
                    ...
                    <div class="col-span-3"><label class="block text-xs text-slate-500 mb-1">Spesifikasi</label><input type="text" name="items[0][specification]" class="w-full px-3 py-2 bg-white border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/30"></div>
                    <div class="col-span-1"><label class="block text-xs text-slate-500 mb-1">Qty</label><input type="number" name="items[0][quantity]" value="1" min="1" required class="w-full px-2 py-2 bg-white border border-slate-200 rounded-lg text-sm text-center focus:outline-none focus:ring-2 focus:ring-blue-500/30"></div>
                    <div class="col-span-1"><label class="block text-xs text-slate-500 mb-1">Satuan</label><input type="text" name="items[0][unit]" value="unit" required class="w-full px-2 py-2 bg-white border border-slate-200 rounded-lg text-sm text-center focus:outline-none focus:ring-2 focus:ring-blue-500/30"></div>
                    <div class="col-span-2"><label class="block text-xs text-slate-500 mb-1">Harga Estimasi</label><input type="number" name="items[0][estimated_price]" min="0" required class="w-full px-2 py-2 bg-white border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/30"></div>
                    <div class="col-span-1 flex justify-center"><button type="button" class="remove-item p-2 text-slate-400 hover:text-red-500 transition-colors" title="Hapus"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg></button></div>
                </div>
            </div>
        </div>
...
document.getElementById('add-item').addEventListener('click', function() {
    ...
    row.innerHTML = `
        ...
        <div class="col-span-3"><label class="block text-xs text-slate-500 mb-1">Spesifikasi</label><input type="text" name="items[${itemIndex}][specification]" class="w-full px-3 py-2 bg-white border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/30"></div>
        <div class="col-span-1"><label class="block text-xs text-slate-500 mb-1">Qty</label><input type="number" name="items[${itemIndex}][quantity]" value="1" min="1" required class="w-full px-2 py-2 bg-white border border-slate-200 rounded-lg text-sm text-center focus:outline-none focus:ring-2 focus:ring-blue-500/30"></div>
        <div class="col-span-1"><label class="block text-xs text-slate-500 mb-1">Satuan</label><input type="text" name="items[${itemIndex}][unit]" value="unit" required class="w-full px-2 py-2 bg-white border border-slate-200 rounded-lg text-sm text-center focus:outline-none focus:ring-2 focus:ring-blue-500/30"></div>
        <div class="col-span-2"><label class="block text-xs text-slate-500 mb-1">Harga Estimasi</label><input type="number" name="items[${itemIndex}][estimated_price]" min="0" required class="w-full px-2 py-2 bg-white border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/30"></div>
