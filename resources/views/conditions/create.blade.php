@extends('layouts.app')
@section('title', 'Catat Pemeriksaan Kondisi')

@section('content')
<div class="mb-6">
    <a href="{{ route('conditions.index') }}" class="text-sm text-blue-600 hover:text-blue-700 font-medium">← Kembali</a>
    <h1 class="text-2xl font-bold text-slate-800 mt-2">Catat Pemeriksaan Kondisi</h1>
</div>
    ...
    <form id="condition-form" class="space-y-5">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
            ...
            <div><label class="block text-sm font-medium text-slate-700 mb-1.5">Kondisi *</label>
                <select name="condition" required class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/30">
                    @foreach(['baik'=>'Baik','rusak_ringan'=>'Rusak Ringan','rusak_berat'=>'Rusak Berat','hilang'=>'Hilang'] as $v=>$l)<option value="{{ $v }}" {{ old('condition') == $v ? 'selected' : '' }}>{{ $l }}</option>@endforeach</select></div>
            <div><label class="block text-sm font-medium text-slate-700 mb-1.5">Tanggal Pemeriksaan *</label>
                <input type="date" name="check_date" value="{{ old('check_date', date('Y-m-d')) }}" required class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/30"></div>
            <div class="md:col-span-2">
                <label class="block text-sm font-medium text-slate-700 mb-1.5">Foto</label>
                <input type="file" name="photo" accept="image/*" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm file:mr-4 file:py-1 file:px-3 file:rounded-lg file:border-0 file:text-xs file:font-medium file:bg-blue-50 file:text-blue-700">
            </div>
        </div>

        <div><label class="block text-sm font-medium text-slate-700 mb-1.5">Deskripsi Kondisi</label><textarea name="description" rows="3" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/30">{{ old('description') }}</textarea></div>
        <div><label class="block text-sm font-medium text-slate-700 mb-1.5">Tindakan yang Diambil</label><textarea name="action_taken" rows="2" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/30">{{ old('action_taken') }}</textarea></div>
        <div class="flex justify-end gap-3 pt-4 border-t border-slate-100">
            <a href="{{ route('conditions.index') }}" class="px-5 py-2.5 text-sm font-medium text-slate-600 bg-slate-100 rounded-xl hover:bg-slate-200 transition-colors">Batal</a>
...
<script>
    ...
    $('#condition-form').on('submit', function(e) {
        e.preventDefault();
        ...
        $.ajax({
            ...
            success: function(res) {
                ...
                setTimeout(() => {
                    window.location.href = '{{ route("conditions.index") }}';
                }, 1500);
            },
            error: function(err) {
